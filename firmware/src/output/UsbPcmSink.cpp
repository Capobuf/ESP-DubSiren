...
bool UsbPcmSink::write(const int16_t *samples, size_t count) {
    ...
    return writePacket(kAudioPacket, reinterpret_cast<const uint8_t *>(samples),
                       byteCount, audioSequence_++);
}
...
bool UsbPcmSink::writePacket(uint8_t type, const uint8_t *payload,
    ...
    const size_t payloadWritten = Serial.write(payload, length);
    xSemaphoreGive(mutex_);
    return headerWritten == sizeof(header) && payloadWritten == length;
}
